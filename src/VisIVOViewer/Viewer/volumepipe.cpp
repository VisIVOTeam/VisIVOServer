...
*/
 ...
 std::clog <<percentileValue[0] << " " << percentileValue[1] << std::endl;
 ...
 std::clog << "Afte range " << m_range[0] << " " << m_range[1] << std::endl;
 //m_range[0] = m_range[0];
 //m_range[0] = -2.95;
 ...
 m_localRange[1] = max;
/*
 vtkImageMathematics *m_math_scale1 = vtkImageMathematics::New();
 m_math_scale1->SetOperationToAddConstant();
 m_math_scale1->SetConstantC(-m_range[0]);
 m_math_scale1->SetInputConnection(threshold2->GetOutputPort());
 float newRange = 9.26e-08 - 5.05e-08;
 double newNorm = newRange/(max - min);
 vtkImageMathematics *m_math_scale2 = vtkImageMathematics::New();


 m_math_scale2->SetOperationToMultiplyByK();
 m_math_scale2->SetConstantK(newNorm);
 m_math_scale2->SetInputConnection(m_math_scale1->GetOutputPort());
 m_math_scale2->Update();
 m_math_scale2->GetOutput()->GetScalarRange(m_range);
 m_math->SetInputConnection(m_math_scale2->GetOutputPort());

 max = m_range[1];
 min = m_range[0];
*/
 m_math->SetInputConnection(threshold2->GetOutputPort());
...
//   if(m_visOpt.isColorRangeTo) max=m_visOpt.colorRangeTo; 
 ...
 //m_localRange[0] = 5.05e-08;
 //m_localRange[1] = 9.26e-08;
double norm = 255.0/(max - min);

std::clog << "min" << min << " norm " << norm << " max - min " << max -min << std::endl;
